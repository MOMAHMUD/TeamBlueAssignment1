...
We decided to implement a K-neareast neighbour (KNN) algorithm to impute missing values. We chose this method because it work with both categorical and continous variables. KNN works by imputing the value based on the majority votes of K nearest neighbours.  To find the nearest neighbours we use a distance function (Euclidean Distance) for continous variables. For our continous variables we need to ensure they aredistributed on the same ranges,we do this by telling the algorithm to re scale all our variables to the same scale. For categorical variables we use Hamming distance.  We chose K to be an odd number as this avoids ties in the voting process.
...
We can see that all the missing data has now been imputed and the correaltion between the variables has not shifted due to the new data.
...
We ran the Random forest (RF) model with 100,1000, 1500 and 2000 trees and compared there accuracy to our RF model with 500 trees and the Decision tree model. We can see the best model was the random forest with 1500 trees. This had an accuracy of `r round(RF1500_confus$overall[1]*100,2)`

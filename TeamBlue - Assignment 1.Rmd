...
Also we realize that converting the variable SEX into M & F would be easier to visulaize instead of "1" and "0" respectively, so we replace them.

```{r, message=FALSE,warning=FALSE}
full_data$SEX<-ifelse(full_data$SEX==1,"M","F")
```

Lastly,we can see that the attributes('SEX','CP','FBS','RESTECG','EXANG','SLOPE','CA','THAL','TARGET') are all factor variables so we conver them into factors
...
and **THAL** with `r round(sum(is.na(full_data$THAL)) / nrow(full_data)*100)` % missing data.  

Other attributes have missing data as well but the issue is not as severe. But we will deal with these under the Data Preparation section.

### Outliers

Based on the boxplots above, we found that we have cholesterol level and resting blood pressure of zero on several data instances in which they don't make sense. We will treat these under the Data Preparation section. 

# Data Preparation

Now that we are done with data exploration we can move on to data preparation.

## Attributes with High Percent of Missing Data
As mentioned above, there are three varibales which have a high percent of missing data we chose to remove them.
...
## Zero Values for Cholesteral and Resting Blood Pressure Values

We assume that the cholesteral and resting blood pressure values of zero are erroneous and we will treat them as missing:
...
We decided to implement a K-neareast neighbour (KNN) algorithm to impute missing values. We chose this method because it work with both categorical and continous variables. KNN works by imputing the value based on the majority votes of K nearest neighbours.  To find the nearest neighbours we use a distance function (Euclidean Distance) for continous variables because we ranges for our continous variables are all different we need to tell the algorithm to re scale all our variables to the samescale. for categorical variables we use Hamming distance.  We chose K to be an odd number as this avoids ties in the voting process.

After implementing KNN we check to see if the correalation between all the variables has shifted at all.  The correlation should not change after imputation and the graph corrgram show this.
...
The most importance variable for prediction our Target are CP with value 4, EXANG with value 1, OLDPEAK, CP with value 2, THALACH, AGE, and SEX with value M.
...
The Random Forest with 500 trees and mtry of`r bestmtry$mtry` had a overall accuracy of  `r round(RF_confus$overall[1]*100,2)` compared to the decision tree model of `r round(confus$overall[1]*100,2)`.  Like the decision tree model the random forest is better at accurately classify target values of 1.  In the above graph the green line shows the error on the TARGET value of 1, the red line shows the error of classify Target value of 0 and the black line shows the Out of bag error of the random forest.  From the chart it looks like the error has converged at around 100 trees, but it does look like it is starting to coverge again around 500 trees. What would happen if we were to add more trees?
...
for (ntree in c(100,1000, 1500, 2000)) {
...
RF100_model.pred <- predict(modellist$`100`, test_data)
RF100_model.pred
# Check Accuracy of Model 
RF100_confus <- confusionMatrix(factor(RF100_model.pred),factor(test_data$TARGET))
...
pred_table <- data.table(Model = c("Decision Tree","Random Forest 100 Trees","Random Forest 500 Trees","Random Forest 1000 Tress","Random Forest 1500 Trees","Random Forest 2000 Trees"),Accuracy = c(confus$overall[1],RF100_confus$overall[1],RF_confus$overall[1],RF1000_confus$overall[1],RF1500_confus$overall[1],RF2000_confus$overall[1]))

pred_table

```

We ran the Random forest (RF) model with 100,1000, 1500 and 2000 trees and compared there accuracy to our RF model with 500 trees and the Decision tree model. We can see the best model was the intial random forest with 500 trees. This had an accuracy of `r round(RF_confus$overall[1]*100,2)`

...
NOw lets us check the outliers in TRESTBPS.
```{r, message=FALSE,warning=FALSE}
...
Based on the boxplots above, we found that we have cholesterol level of zero on several data instances in which they don't make sense. We will treat these under the Data Preparation section. 
...
## Zero Values for Cholesteral Values

We assume that the cholesteral values of zero are erroneous and we will treat them as missing:
...
We decided to implement a K-neareast neighbour (KNN) algorithm to impute missing values. We chose this method because it work with both categorical and continous variables. KNN wokrs by imputing the value based on the majority votes of K nearest neighbours.  To find the nearest neighbours we use a distance function (Euclidean Distance) for continous variables because we ranges for our continous variables are all different we need to tell the algorithm to re scale all our variables to the samescale. for categorical variables we use Hamming distance.  We chose K to be an odd number as this avoids ties in the voting process.
